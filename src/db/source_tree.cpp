                                          ...
                                          Sched::ThreadPool& workers)
{
	...
	visitNodesInTopoOrder(
	   [&graph = src_tree.dependencyGraph(), &build_info, &commands, force_recompilation, &ctxt](
	      auto const& node) {
		   ctxt.add(node.id().value(),
		            [&graph, &node, &build_info, &commands, force_recompilation, &ctxt]() {
			            compile(graph, node, build_info, commands, force_recompilation, ctxt);
		            });
	   },
                                          ...
                                          std::pair<fs::path const*, size_t> targets)
{
	...
	std::for_each_n(targets.first,
	                targets.second,
	                [&n, &src_tree, &build_info, &commands, force_recompilation, &workers](
	                   auto const& target_name) {
		                n += compile(
		                   src_tree, build_info, commands, force_recompilation, workers, target_name);
	                });
