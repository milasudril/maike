...
namespace Maike
{
	...
	struct PRIVATE gen_seq: gen_seq<N - 1, N - 1, Is...>
	{
	};
	template<size_t... Is>
	struct PRIVATE gen_seq<0, Is...>: seq<Is...>
	{
	};
	...
	constexpr c_array<T, Size>
	c_swap(c_array<T, Size> const& arr, size_t index0, size_t index1, seq<Is...>)
	{
	...
	public:
		...
		static Iterator find(const Key& key) noexcept
		{
			...
			return (first != keys.end() && !(key < *first)) ? Iterator(first - keys.begin()) : end();
		}
	...
	private:
#ifndef DOXYGEN
		static constexpr c_array<Key, sizeof...(args)> keys{
		   c_sel_sort(c_array<Key, sizeof...(args)>(Key(args)...))};
		...
		struct PRIVATE LowerBound
		{
			...
			static constexpr size_t count_next = keys[pos] < key ? count - (step + 1) : step;
		...
		struct PRIVATE Find
		{
			static constexpr size_t value =
			   IfFalse < LowerBound<key>::value == sizeof...(args)
			   || key<keys[LowerBound<key>::value], LowerBound<key>::value>::value;
		};
	};
	template<class Key, class Value, Key... args>
