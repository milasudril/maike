...
namespace Maike
{
	...
	public:
		...
		explicit constexpr Stringkey(const char* str) noexcept: m_value(hash(str))
		{
		}
	...
	private:
		...
		static constexpr uint64_t hash(const char* str, HashValue value_init = OFFSET_BASIS)
		{
			return *str == '\0' ? value_init : hash(str + 1, (value_init ^ (HashValue(*str))) * FNV_PRIME);
		}
	};
}
