...
static WriteBuffer& escape(WriteBuffer& wb, const char* str)
{
	...
	while(*str != '\0')
	{
		...
		{
			case '\'': wb.write("'\\''"); break;
			default: wb.write(static_cast<uint8_t>(ch_in)); break;
		}
		++str;
	}
	wb.write(static_cast<uint8_t>('\''));
...
static void commandLinePrint(const char* const* cmdline, bool use_stdin)
{
	...
	while(*cmdline != nullptr)
	{
		...
		if(*cmdline == nullptr)
		{
			if(use_stdin) { wb.write(" << '").write(HEREDOC).write("'"); }
			wb.write("\n");
		}
		else
		{
...
Pipe::Pipe(const char* command, Twins<const char* const*> args, unsigned int flags)
{
	sigpipeSetFilter();

	PipeHelper stdin_pipe;
	if(flags & REDIRECT_STDIN) { stdin_pipe.init(); }

	PipeHelper stdout_pipe;
	if(flags & REDIRECT_STDOUT) { stdout_pipe.init(); }

	PipeHelper stderr_pipe;
	if(flags & REDIRECT_STDERR) { stderr_pipe.init(); }

	PipeHelper exec_error;
	exec_error.init();

	auto args_out = commandLineBuild(command, args);
	if(!(flags & ECHO_OFF)) { commandLinePrint(args_out.data(), flags & REDIRECT_STDIN); }
	...
	if(child.pidGet() == 0)
	{
		...
		if(flags & REDIRECT_STDIN) { dup2(stdin_pipe.readEndRelease(), STDIN_FILENO); }

		if(flags & REDIRECT_STDOUT) { dup2(stdout_pipe.writeEndRelease(), STDOUT_FILENO); }

		if(flags & REDIRECT_STDERR) { dup2(stderr_pipe.writeEndRelease(), STDERR_FILENO); }
	...
	if(flags & REDIRECT_STDIN) { m_stdin.init(stdin_pipe.writeEndRelease(), !(flags & ECHO_OFF)); }

	if(flags & REDIRECT_STDOUT) { m_stdout.init(stdout_pipe.readEndRelease()); }

	if(flags & REDIRECT_STDERR) { m_stderr.init(stderr_pipe.readEndRelease()); }
...
Pipe::Reader::Reader(Reader&& reader) noexcept: m_handle(reader.m_handle)
{
	reader.m_handle = -1;
}
...
Pipe::Writer::Writer(Writer&& writer) noexcept: m_handle(writer.m_handle), m_echo(writer.m_echo)
{
	writer.m_handle = -1;
}
...
size_t Pipe::Writer::write(const void* buffer, size_t count)
{
	...
	while(n != count)
	{
		...
		if(res == -1)
		{
			if(errno == ENOSPC || errno == EPIPE) { return n; }
			if(!(errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK))
			{ exceptionRaise(ErrorMessage("I/O error", {})); }
			n = 0;
		}
