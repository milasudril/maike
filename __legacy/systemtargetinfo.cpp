...
SystemTargetInfo& SystemTargetInfo::configAppend(const ResourceObject& targetinfo)
{
	...
	while(!i.endAt())
	{
		...
		{
			case ResourceObject::Type::ARRAY: replace(m_sysvars, {key, "[...]"}); break;
			...
			case ResourceObject::Type::INTEGER:
				...
				break;
			case ResourceObject::Type::OBJECT: replace(m_sysvars, {key, "{...}"}); break;
...
void SystemTargetInfo::parameterGet(const Stringkey& key, ParameterProcessor&& proc) const
{
	auto i = m_sysvars.find(key);
	if(i != m_sysvars.end())
	{
		auto& var = i->second;
		if(var.typeGet() == Variant::STRING) { proc(static_cast<const char*>(var)); }
		else
		{
			char buffer[32];
...
{
	auto i = m_sysvars.find(key);
	if(i == m_sysvars.end()) { return static_cast<int64_t>(0); }
	return i->second;
}
