...
namespace Maike
{
	...
	public:
		enum class Relation : unsigned int
		{
			...
			,
			INCLUDE_EXTRA /**<This is similar to INCLUDE, but the file will passed to the compiler on the
			                 command line.*/
			,
			IMPLEMENTATION /**<The secondary Target should be used when linking or archiving the primary
			                  Target.*/
			,
			EXTERNAL /**<The secondary Target is unknown to Maike*/
			,
			TOOL /**<This relation is similar to EXTERNAL, but is used to annotate that the secondary Target
			        is a tool*/
			,
		...
		explicit Dependency(Target& target, Relation rel): m_name(nullptr), r_target(&target), m_rel(rel)
		{
		}
		...
		{
			m_name = nullptr;
			if(obj.m_name == nullptr) { r_target = obj.r_target; }
			else
			{
				nameSet(obj.m_name);
				r_target = nullptr;
		...
		{
			return r_target == nullptr ? m_name : r_target->nameGet();
		}
	...
	static constexpr int USE_ALL = USE_EXTERNAL | USE_IMPLEMENTATION | USE_INCLUDE | USE_INCLUDE_EXTRA
	                               | USE_MISC | USE_LEAF | USE_INTERNAL | USE_TOOL;

	inline std::string name_full_get(const char* target_dir, const Target& t)
	{
		if(t.generated()) { return dircat(target_dir, t.nameGet()); }
		return t.nameGet();
	}

	template<class Proc>
	static bool dependenciesProcess(const char* target_dir,
	                                ...
	                                Proc&& proc)
	{
		while(deps.first != deps.second)
		{
			...
			if(rel == Dependency::Relation::EXTERNAL && (use_flags & USE_EXTERNAL))
			{
				...
				if(!proc(name_full.c_str(), rel)) { return 0; }
			}
			...
			if(rel == Dependency::Relation::INTERNAL && (use_flags & USE_INTERNAL))
			{
				...
				if(!proc(t->nameGet(), rel)) { return 0; }
			}
