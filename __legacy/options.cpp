...
{
	MAKE_OPTION(
	   options, 0, "help", "Print this message to `stdout`, or to the given file, and exit.", 1);
	...
	MAKE_OPTION(options,
	            1,
	            "configfiles",
	            "Load the listed configuration files. "
	...
	MAKE_OPTION(options,
	            1,
	            "no-sysvars",
	            "Do not load any default system variables. "
	            "This option is useful when using a cross-compiler. Normally, Maike loads a "
	            "system-dependent set of variables that identifies the host platform.",
	            0);
	...
	MAKE_OPTION(
	   ...
	   1,
	   "configclean",
	   "Do not load any system specific default configuration. "
	   "In this mode, Maike will only load its build-in configuration, either followed by the file "
	   "`maikeconfig.json`, if present in the current directory, or the files listed in "
	   "`configfiles`. "
	   "If the option `configfiles` is given, `maikeconfig.json` will not be loaded.",
	   0);

	MAKE_OPTION(options,
	            1,
	            "hooks-load",
	            "Explicitly load the given target hook plugins. The string follows the "
	            "syntax `hook_plugin`:[`filename_ext`,...].",
	            2);

	MAKE_OPTION(options,
	            1,
	            "hooks-config",
	            "Set properties for the given target hooks. The string follows the "
	            "syntax `hook_name`:[`JSON string without surrounding curly braces`]. ",
	            2);
	...
	MAKE_OPTION(options,
	            2,
	            "targets",
	            "Only process the listed targets. This "
	...
	MAKE_OPTION(options,
	            2,
	            "clean",
	            "Remove targets from disk. This option "
	            "is like a conventional `make clean`, with the addition that "
	            "the affected targets can be controlled by the `targets` option.",
	            0);

	MAKE_OPTION(options,
	            2,
	            "remove-orphans",
	            "Remove targets *not* known by Maike, "
	...
	MAKE_OPTION(options,
	            2,
	            "list-leaf-targets",
	            "Print all leaf targets to the "
	            "given file and exit. As default, data is written to `stdout`. A leaf "
	            "target is a target that no other target refers to.",
	            1);

	MAKE_OPTION(options,
	            2,
	            "list-external-targets",
	            "Print all external targets to "
	...
	MAKE_OPTION(options,
	            ...
	            "list-all-targets",
	            "Print all targets "
	            "to the given file and exit. As default, the data is written to `stdout`.",
	            1);

	MAKE_OPTION(options,
	            3,
	            "dump-graph-dot",
	            "Dump the dependency graph to a DOT "
	            "file and exit. As default, the data is written to `stdout`.",
	            1);

	MAKE_OPTION(options,
	            3,
	            "dump-graph-inv-dot",
	            "Dump an inverted dependency graph "
	            "to a DOT file and exit. As default, the data is written to `stdout`.",
	            1);

	MAKE_OPTION(options,
	            3,
	            "dump-database-json",
	            "Dump the internal database to a JSON "
	            "file and exit. As default, the data is written to `stdout`.",
	            1);
	...
	MAKE_OPTION(options,
	            3,
	            "dump-targets-tsv",
	            "Dumps information about the selected "
	...
	MAKE_OPTION(options,
	            0,
	            "about",
	            "Print information about maike  to `stdout`, or to the given file, and exit.",
	            1);
}

static void
optionLoad(Options::OptionMap& options, std::vector<std::string>* strings, const char* arg)
{
	std::string buffer;
	...
	while(true)
	{
		...
		switch(state)
		{
			case State::START_0:
			{
				switch(ch_in)
				{
					case '-': state = State::START_1; break;
					default: exceptionRaise(ErrorMessage("Command line error: Expected `-` got #0;.", {ch_in}));
				}
			}
			break;
			...
			case State::KEY:
			{
				switch(ch_in)
				{
					case '=':
					{
						...
						{
							exceptionRaise(ErrorMessage("Command line error: Option #0; does not take any arguments.",
							                            {buffer.c_str()}));
						}
					...
					case '\0':
					{
						...
						{
							exceptionRaise(ErrorMessage("Command line error: Option #0; requires at least 1 value.",
							                            {buffer.c_str()}));
						}
						val_current = &strings[static_cast<size_t>(i)];
						option_current->values = val_current;
			...
			case State::VALUE:
				switch(ch_in)
				{
					...
					case ']':
						...
						if(brackets != 0) { buffer += ch_in; }
						break;
					case '\\': state = State::VALUE_ESCAPE; break;
					case ',':
						if(option_current->argcount < 2 && val_current->size() == option_current->argcount)
						{
							exceptionRaise(ErrorMessage("Command line error: Option #0; only takes 1 value.",
							                            {option_current->key}));
						}
						if(brackets == 0)
						{
...
Options::Options(Twins<const char* const*> args)
{
	...
	if(args.first != args.second) { ++args.first; }
...
static bool optionCompare(const Options::Option& a, const Options::Option& b)
{
	...
	if(a.group == b.group && strcmp(a.key, b.key) < 0) { return 1; }
	return 0;
}
...
void Options::printHelpImpl(DataSink& sink) const
{
	...
	std::copy(m_options.values(), m_options.values() + N_values, options_sorted);
