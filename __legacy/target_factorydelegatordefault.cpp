                                                          ...
                                                          const Dependency& dependency)
{
	...
	if(ptr_ret != nullptr) { return *ptr_ret; }
	   ...
	   || dependency.relationGet() == Dependency::Relation::EXTERNAL_RESOURCE)
	{
		...
		if(dependency.relationGet() == Dependency::Relation::TOOL)
		{
			...
			{ exceptionRaise(ErrorMessage("#0;: Dependency #1; is not satisfied", {target_from, name})); }
		}
...
Target_FactoryDelegatorDefault&
                                                            ...
                                                            size_t line_count)
{
	...
	{
		exceptionRaise(
		   ErrorMessage("#0;: It is impossible to choose a target factory "
	...
	{
		exceptionRaise(
		   ErrorMessage("#0;: #1; is not associated with any target factory", {name_src, suffix}));
	}
	...
	try
	{
		switch(FileInfo(filename.c_str()).typeGet())
		{
			case FileInfo::Type::FILE:
			{
				...
				if(pos == std::string::npos) { return Stringkey(""); }
				return Stringkey(&filename[pos]);
			}

			case FileInfo::Type::DIRECTORY: return Stringkey(".");

			default: return Stringkey("");
		}
	}
	catch(...)
	{
...
static const Target_Loader*
targetLoaderGet(const Stringkey& key, const std::map<Stringkey, const Target_Loader*>& loaders)
{
	auto i = loaders.find(key);
	if(i == loaders.end()) { return nullptr; }
	return i->second;
}
                             ...
                             DependencyBuffer& buffer)
{
	...
	if(loader != nullptr)
	{
		...
		if(i == deps_extra_cache.end())
		{
			...
			loader->dependenciesExtraGet(dep_in.nameGet(),
			                             in_dir_include.c_str(),
			                             root,
			                             ResourceObjectJansson::createImpl,
			                             buffer_temp);
...
static void dependenciesCollect(const char* name_src,
                                const char* in_dir,
                                const char* root,
                                Spider& spider,
                                ...
                                std::map<Stringkey, DependencyBufferDefault>& deps_extra_cache)
{
	...
	loader_current.dependenciesGet(
	   name_src, in_dir, root, ResourceObjectJansson::createImpl, buffer_tmp);

	std::for_each(buffer_tmp.begin(),
	              buffer_tmp.end(),
...
static void targetsCreate(const ResourceObject& targets,
                          const char* name_src,
                          const char* in_dir,
                          size_t line_count,
                          ...
                          std::map<Stringkey, DependencyBufferDefault>& deps_extra_cache)
{
	...
	dependenciesCollect(
	   name_src, in_dir, root, spider, loader_current, loaders, deps, deps_extra_cache);
                                                       ...
                                                       DependencyGraph& graph)
{
	...
	{
		::targetsCreate(obj.objectGet("targets"),
		                name_src,
		                in_dir,
		                line_count,
		                *this,
	...
	else
	{
		...
		for(decltype(N_cases) k = 0; k < N_cases; ++k)
		{
			...
			if(case_obj.typeGet() == ResourceObject::Type::ARRAY)
			{
				if(case_obj.objectCountGet() != 2)
				{
					exceptionRaise(ErrorMessage(
					   "#0;: A condition must have only a condition and a target definition.", {name_src}));
				}
			...
			else if(case_obj.objectExists("targets"))
			{
				::targetsCreate(case_obj.objectGet("targets"),
				                name_src,
				                in_dir,
				                line_count,
				                *this,
...
void Target_FactoryDelegatorDefault::targetsLoad(const char* filename,
                                                 const char* in_dir,
                                                 Spider& spider,
                                                 DependencyGraph& targets)
{
	auto loader = targetLoaderGet(targetLoaderKeyGet(filename), m_r_loaders);
	if(loader != nullptr) { loader->targetsLoad(filename, in_dir, spider, targets, *this); }
}
