...
{
	if(filename.size() == 0) { return FileOut(); }
...
static std::vector<std::string> hookDataExtract(const char* str)
{
	...
	while(1)
	{
		...
		{
			case ',':
				if(ret.size() == 0) { exceptionRaise(ErrorMessage("Missing hook filename", {})); }
			...
			case ':':
				...
				buffer.clear();
				break;
			case '\0': ret.push_back(buffer); return ret;
...
static int targetsListExternal(const Maike::Session& maike,
...
static int databaseDumpJSON(Maike::Session& maike,
                            ...
                            const std::vector<std::string>& filename)
{
	if(targets == nullptr) { targetsDump(maike).write(fileGet(filename)); }
	else
	{
		auto ptr = targets->data();
		auto ptr_end = ptr + targets->size();
                          ...
                          const std::vector<std::string>& filename)
{
	...
	if(targets == nullptr) { targetsDumpTSV(maike, file); }
	else
	{
		auto ptr = targets->data();
		auto ptr_end = ptr + targets->size();
...
int main(int argc, char** argv)
{
	try
	{
		...
		if(x != nullptr) { return helpPrint(opts, *x); }

		x = opts.get<Stringkey("version")>();
		if(x != nullptr) { return versionPrint(*x); }


		x = opts.get<Stringkey("about")>();
		if(x != nullptr) { return about(*x); }

		x = opts.get<Stringkey("load-path")>();
		if(x != nullptr) { return loadPathPrint(*x); }

		if(opts.get<Stringkey("no-sysvars")>() == nullptr) { sysvarsLoad(maike); }
		...
		if(x != nullptr) { hooksLoad(maike, *x); }

		x = opts.get<Stringkey("hooks-config")>();
		if(x != nullptr) { hooksConfig(maike, *x); }

		x = opts.get<Stringkey("configdump")>();
		if(x != nullptr) { return configDump(maike, *x); }

		x = opts.get<Stringkey("list-all-targets")>();
		if(x != nullptr) { return targetsListAll(maike, *x); }

		x = opts.get<Stringkey("list-leaf-targets")>();
		if(x != nullptr) { return targetsListLeaf(maike, *x); }

		x = opts.get<Stringkey("list-leaf-targets")>();
		if(x != nullptr) { return targetsListLeaf(maike, *x); }

		x = opts.get<Stringkey("list-external-targets")>();
		if(x != nullptr) { return targetsListExternal(maike, *x); }

		x = opts.get<Stringkey("dump-graph-dot")>();
		if(x != nullptr) { return graphDumpDOT(maike, opts.get<Stringkey("targets")>(), *x); }

		x = opts.get<Stringkey("dump-graph-inv-dot")>();
		if(x != nullptr) { return graphInvDumpDOT(maike, opts.get<Stringkey("targets")>(), *x); }

		x = opts.get<Stringkey("dump-database-json")>();
		if(x != nullptr) { return databaseDumpJSON(maike, opts.get<Stringkey("targets")>(), *x); }

		x = opts.get<Stringkey("remove-orphans")>();
		if(x != nullptr) { removeOrphans(maike); }

		x = opts.get<Stringkey("clean")>();
		if(x != nullptr) { return clean(maike, opts.get<Stringkey("targets")>()); }
		...
		if(x != nullptr) { return targetsDumpTSV(maike, opts.get<Stringkey("targets")>(), *x); }
	}
