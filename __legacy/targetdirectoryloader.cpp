                                        ...
                                        Target_FactoryDelegator& factory) const
{
	...
	while(entry != nullptr)
	{
		if(m_ignore.find(Stringkey(entry)) == m_ignore.end())
		{
			...
			if(m_ignore_fullname.find(Stringkey(path_tot.c_str())) == m_ignore_fullname.end())
			{
...
{
	directoryoptions.objectSet("recursive",
	                           directoryoptions.create(static_cast<long long int>(m_recursive)));
	{
