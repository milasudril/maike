...
template<class T>
static char*
convert_impl(char* buffer, typename std::make_unsigned<T>::type x, unsigned int radix) noexcept
{
	auto begin = buffer;
	while(x)
	{
...
static size_t format(const Twins<char*>& range, const void* ptr, const Twins<const char*>&) noexcept
{
...
static size_t
format(const Twins<char*>& range, const char* cstr, const Twins<const char*>&) noexcept
{
			...
			case State::NORMAL:
				switch(ch_in)
				{
					...
					case 'h':
						...
						state = State::DONE;
						break;
					case ':': state = State::ARGS_BEGIN; break;
...
static size_t
format(const Twins<char*>& range, int64_t x, const Twins<const char*>& argstring) noexcept
{
...
static size_t
format(const Twins<char*>& range, uint64_t x, const Twins<const char*>& argstring) noexcept
{
	auto radix = radixGet(argstring);
...
static size_t
format(const Twins<char*>& range, char x, const Twins<const char*>& argstring) noexcept
{
	size_t N = range.second - range.first;
	if(N)
	{
		...
		if(N)
		{
			...
			{
				return format(
				   range, static_cast<int64_t>(x), Twins<const char*>(argstring.first + 1, argstring.second));
			}
		}
		range.first[0] = x;
		return 1;
...
static size_t format(const Twins<char*>& range, bool x, const Twins<const char*>&) noexcept
{
	auto pos = range.first;
	size_t N = range.second - range.first;
	if(x)
...
static std::pair<char, unsigned int> floatParams(const Twins<const char*>& argstring,
                                                 unsigned int digits_default,
                                                 unsigned int digits_max) noexcept
{
	...
	while(args_begin != args_end)
	{
		...
		switch(state)
		{
			case State::NORMAL:
				state = State::DIGITS;
				if(ch_in != ',' && ch_in != '.') { return {separator, n_digits}; }
				separator = ch_in;
...
static size_t
format(const Twins<char*>& range, double x, const Twins<const char*>& argstring) noexcept
{
	...
	while(pos != pos_end && *ptr_source != '\0')
	{
		...
		if(ch_in == ',' || ch_in == '.') { ch_in = params.first; }
		*pos = ch_in;
		++ptr_source;
		++pos;
	}
...
static size_t
format(const Twins<char*>& range, float x, const Twins<const char*>& argstring) noexcept
{
...
static char*
process(const Twins<char*>& range, const Variant& var, const Twins<const char*>& argstring) noexcept
{
	auto pos = range.first;
	switch(var.typeGet())
	{
		case Variant::POINTER: pos += format(range, static_cast<const void*>(var), argstring); break;
		case Variant::STRING: pos += format(range, static_cast<const char*>(var), argstring); break;
		case Variant::INT: pos += format(range, static_cast<int64_t>(var), argstring); break;
		case Variant::UINT: pos += format(range, static_cast<uint64_t>(var), argstring); break;
		case Variant::CHAR: pos += format(range, static_cast<char>(var), argstring); break;
		case Variant::BOOL: pos += format(range, static_cast<bool>(var), argstring); break;
		case Variant::DOUBLE: pos += format(range, static_cast<double>(var), argstring); break;
		case Variant::FLOAT: pos += format(range, static_cast<float>(var), argstring); break;
...
void Maike::format(const Twins<char*>& buffer,
	...
	while(write_pos != write_end && *format_string != '\0')
	{
		...
		switch(state)
		{
			case State::NORMAL:
				switch(ch_in)
				{
					case '#': state = State::PLACEHOLDER_BEGIN; break;
					default: *write_pos = ch_in; ++write_pos;
				}
				break;
				...
				if(!(ch_in >= '0' && ch_in <= '9'))
				{
					...
					else if(ch_in == ';')
					{
						...
						write_pos = process(Twins<char*>(write_pos, write_end),
						                    args.first[index_current],
						                    Twins<const char*>(format_string, format_string));
					}
					else
					{
			...
			case State::PLACEHOLDER_ARGSTRING:
				if(ch_in == ';')
				{
					...
					write_pos = process(Twins<char*>(write_pos, write_end),
					                    args.first[index_current],
					                    Twins<const char*>(argpos, format_string));
				}
				break;
		}
		++format_string;
	}
	if(write_pos != write_end) { *write_pos = '\0'; }
	*(write_end - 1) = '\0';
}
