...
namespace
{
	...
	static Variant argCreate(const char* value,
	                         bool string_is,
			...
			if(*ptr_end != '\0')
			{
				exceptionRaise(
				   ErrorMessage("Expression error: "
				                "\"#0;\" is not a valid floating point number.",
				                {value}));
			}
			return x;
		}
	...
	int64_t and_(Twins<const Variant*> values, uintptr_t type_tag)
	{
		while(values.first != values.second)
		{
			if(values.first->typeGet() != type_tag)
			{ exceptionRaise(ErrorMessage("Expression error: Inconsistent types.", {})); }
			if(static_cast<T>(*values.first) == 0) { return 0; }
			++values.first;
		}
		return 1;
	}
	...
	int64_t or_(Twins<const Variant*> values, uintptr_t type_tag)
	{
		while(values.first != values.second)
		{
			if(values.first->typeGet() != type_tag)
			{ exceptionRaise(ErrorMessage("Expression error: Inconsistent types.", {})); }
			if(static_cast<T>(*values.first) != 0) { return 1; }
			++values.first;
		}
		return 0;
	}
	...
	Variant execute(const Command& cmd)
	{
		...
		{
			exceptionRaise(
			   ErrorMessage("Expression error: A function needs at "
			                "least one argument.",
			                {}));
		}
		...
		if(cmd.name == Stringkey("add"))
		{
			...
			switch(type)
			{
				case Variant::DOUBLE: return add<double>({ptr, ptr + cmd.args.size()}, type);
				case Variant::INT: return add<int64_t>({ptr, ptr + cmd.args.size()}, type);
				default:
					exceptionRaise(
					   ErrorMessage("Expression error: The 'add' function "
					                "requires either an int or a double.",
					                {}));
			}
		}
		else if(cmd.name == Stringkey("multiply"))
		{
			auto ptr = cmd.args.data();
			auto type = ptr->typeGet();
			switch(type)
			{
				case Variant::DOUBLE: return multiply<double>({ptr, ptr + cmd.args.size()}, type);
				case Variant::INT: return multiply<int64_t>({ptr, ptr + cmd.args.size()}, type);
				default:
					exceptionRaise(
					   ErrorMessage("Expression error: The 'multiply' function "
					                "requires either an int or a double.",
					                {}));
			}
		}
		else if(cmd.name == Stringkey("and"))
		{
			auto ptr = cmd.args.data();
			auto type = ptr->typeGet();
			switch(type)
			{
				case Variant::INT: return and_<int64_t>({ptr, ptr + cmd.args.size()}, type);
				case Variant::BOOL: return and_<bool>({ptr, ptr + cmd.args.size()}, type);
				default:
					exceptionRaise(
					   ErrorMessage("Expression error: The 'and' function "
					                "requires either an int or a bool.",
					                {}));
			}
		}
		else if(cmd.name == Stringkey("or"))
		{
			auto ptr = cmd.args.data();
			auto type = ptr->typeGet();
			switch(type)
			{
				case Variant::INT: return or_<int64_t>({ptr, ptr + cmd.args.size()}, type);
				case Variant::BOOL: return or_<bool>({ptr, ptr + cmd.args.size()}, type);
				default:
					exceptionRaise(
					   ErrorMessage("Expression error: The 'or' function "
					                "requires either an int or a bool.",
					                {}));
			}
		}
		else if(cmd.name == Stringkey("not"))
		{
			auto ptr = cmd.args.data();
			auto type = ptr->typeGet();
			...
			switch(type)
			{
				case Variant::INT: return static_cast<int64_t>(!static_cast<int64_t>(ptr[0]));
				case Variant::BOOL: return !static_cast<bool>(ptr[0]);
				default:
					exceptionRaise(
					   ErrorMessage("Expression error: The 'not' function "
					                "requires either an int or a bool.",
					                {}));
			}
		...
		else if(cmd.name == Stringkey("less_than"))
		{
			auto N = cmd.args.size();
			if(N < 2)
			{
				exceptionRaise(
				   ErrorMessage("Expression error: The 'less_than' function "
			...
			switch(type)
			{
				case Variant::DOUBLE: return less_than<double>({ptr, ptr + N}, type);
				case Variant::INT: return less_than<int64_t>({ptr, ptr + N}, type);
				default:
					exceptionRaise(
					   ErrorMessage("Expression error: The 'less_than' function "
					                "requires either an int or a double.",
		...
		else if(cmd.name == Stringkey("greater_than"))
		{
			auto N = cmd.args.size();
			if(N < 2)
			{
				exceptionRaise(
				   ErrorMessage("Expression error: The 'greater_than' function "
				                "requires at least two arguments.",
				                {}));
			}
			auto ptr = cmd.args.data();
			...
			switch(type)
			{
				case Variant::DOUBLE: return greater_than<double>({ptr, ptr + N}, type);
				case Variant::INT: return greater_than<int64_t>({ptr, ptr + N}, type);
				default:
					exceptionRaise(
					   ErrorMessage("Expression error: The 'greater_than' function "
					                "requires either an int or a double.",
		...
		else if(cmd.name == Stringkey("equals"))
		{
			auto N = cmd.args.size();
			if(N < 2)
			{
				exceptionRaise(
				   ErrorMessage("Expression error: The 'equals' function "
				                "requires at least two arguments.",
				                {}));
			}
			auto ptr = cmd.args.data();
			...
			switch(type)
			{
				case Variant::INT: return equals<int64_t>({ptr, ptr + N}, type);
				case Variant::BOOL: return equals<int64_t>({ptr, ptr + N}, type);
				default:
					exceptionRaise(
					   ErrorMessage("Expression error: The 'equals' function "
					                "requires either an int or a bool.",
		...
		else if(cmd.name == Stringkey("version"))
		{
			auto ptr = cmd.args.data();
		...
		else if(cmd.name == Stringkey("file_exists"))
		{
			auto ptr = cmd.args.data();
			if(ptr->typeGet() != Variant::STRING)
			...
			case State::NORMAL:
			{
				switch(ch_in)
				{
					...
					case '\0':
						...
						{
							ret = argCreate(tok_current.first.c_str(), tok_current.second, strings, *this);
						...
						if(ch_in == '\0')
						{
							if(!cmdstack.empty()) { exceptionRaise(ErrorMessage("Expression error: Missing ')'.", {})); }
							return ret;
						}
						break;
					case ')':
						if(cmdstack.empty())
						{
							exceptionRaise(
							   ErrorMessage("Expression error: "
							                "Unexpected ')' after #0;.",
							                {tok_current.first.c_str()}));
						}
						if(tok_current.first.size() != 0)
						{
							cmd_current.args.push_back(
							   argCreate(tok_current.first.c_str(), tok_current.second, strings, *this));
						}
					...
					case '\\':
						...
						break;
					default:
						if(!(ch_in >= '\0' && ch_in <= ' ')) { tok_current.first += ch_in; }
				};
			}
			break;
					...
					case '\0':
						exceptionRaise(
						   ErrorMessage("Expression error: "
						                ...
						                {}));
					default: tok_current.first += ch_in;
				};
			}
			break;
