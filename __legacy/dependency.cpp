...
static Dependency::Relation relation(const char* string)
{
	...
	if(key == Stringkey("implementation")) { return Dependency::Relation::IMPLEMENTATION; }
	if(key == Stringkey("external")) { return Dependency::Relation::EXTERNAL; }
	if(key == Stringkey("internal")) { return Dependency::Relation::INTERNAL; }
	if(key == Stringkey("leaf")) { return Dependency::Relation::LEAF; }
	if(key == Stringkey("generated"))
	{
		WriteBuffer(StdStream::error())
		   .write(
		      "Warning: The relation `generated` is deprecated. "
		      "Use `misc` instead.\n");
	...
	if(key == Stringkey("include_generated"))
	{
		WriteBuffer(StdStream::error())
		   .write(
		      "Warning: The relation `include_generated` is deprecated. "
		      "Use `include_extra` instead.\n");
		return Dependency::Relation::INCLUDE_EXTRA;
	}
	if(key == Stringkey("include_extra")) { return Dependency::Relation::INCLUDE_EXTRA; }
	if(key == Stringkey("file"))
	{
		WriteBuffer(StdStream::error())
		   .write(
		      "Warning: The relation `file` is deprecated. "
		      "Use `misc` instead.\n");
		return Dependency::Relation::MISC;
	}
	if(key == Stringkey("include")) { return Dependency::Relation::INCLUDE; }
	if(key == Stringkey("tool")) { return Dependency::Relation::TOOL; }
	if(key == Stringkey("misc")) { return Dependency::Relation::MISC; }
	if(key == Stringkey("runtime")) { return Dependency::Relation::RUNTIME; }
	if(key == Stringkey("external_resource")) { return Dependency::Relation::EXTERNAL_RESOURCE; }
...
static const char* relation(Dependency::Relation rel)
{
	switch(rel)
	{
		case Dependency::Relation::IMPLEMENTATION: return "implementation";
		case Dependency::Relation::EXTERNAL: return "external";
		case Dependency::Relation::INTERNAL: return "internal";
		case Dependency::Relation::LEAF: return "leaf";
		case Dependency::Relation::MISC: return "misc";
		case Dependency::Relation::INCLUDE: return "include";
		case Dependency::Relation::TOOL: return "tool";
		case Dependency::Relation::INCLUDE_EXTRA: return "include_extra";
		case Dependency::Relation::RUNTIME: return "runtime";
		case Dependency::Relation::EXTERNAL_RESOURCE: return "external_resource";
	}
	return nullptr;
}
...
void Dependency::nameSet(const char* name, size_t size)
{
	...
	if(m_name == NULL) { exceptionRaise(ErrorMessage("Out of memory", {})); }
