...
Session& Session::rootSet(const char* root)
{
	...
	m_target_dir_full =
	   dircat(root, static_cast<const char*>(m_targetinfo.variableGet(Stringkey("target_directory"))));
	graphDirtySet();
	return *this;
}
...
Session& Session::scanFile(const char* filename)
{
	...
	if(targetHooksDirty()) { targetHooksRegister(); }
...
{
	if(graphDirty()) { dependenciesReload(); }
...
const Target& Session::target(const char* name) const
{
	...
	if(ret == nullptr) { exceptionRaise(ErrorMessage("Target #0; has not been loaded.", {name})); }
	return *ret;
}
