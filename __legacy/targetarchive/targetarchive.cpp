...
static TargetArchive::Type type(const char* name_src, const char* type)
{
	...
	if(key == Stringkey("tar")) { return TargetArchive::Type::TAR; }

	if(key == Stringkey("zip")) { return TargetArchive::Type::ZIP; }
...
static const char* type(TargetArchive::Type type)
{
	switch(type)
	{
		case TargetArchive::Type::TAR: return "tar";
		case TargetArchive::Type::ZIP: return "zip";
	}
	return nullptr;
}

TargetArchive::TargetArchive(const ResourceObject& obj,
                             const TargetArchiveCompiler& compiler,
                             const char* name_src,
                             const char* in_dir,
                             const char* root,
                             size_t id,
   ...
   TargetBase(obj, name_src, in_dir, root, id, line_count), r_compiler(compiler)
{
	...
	if(obj.objectExists("contents"))
	{
		...
		for(decltype(N) n = 0; n < N; ++n)
		{
			...
			dependencyAdd(Dependency(filename_full.c_str(), root, Dependency::Relation::MISC));
		}
	}
}
	...
	while(dependency_list.first != dependency_list.second)
	{
		...
		switch(dep->relationGet())
		{
			case Dependency::Relation::GENERATED:
				if(FileUtils::newer(dircat(target_dir, dep->nameGet()).c_str(), name_full.c_str()))
				{ return 0; }
				break;
			case Dependency::Relation::FILE:
				if(FileUtils::newer(dep->nameGet(), name_full.c_str())) { return 0; }
				break;
			default: break;
		}
                                ...
                                const char* target_dir)
{
	...
	auto range =
	   Twins<const char* const*>(r_files_input.data(), r_files_input.data() + r_files_input.size());
	switch(typeGet())
	{
		case Type::TAR: r_compiler.tar(range, nameGet(), target_dir, rootGet(), compressionGet()); break;
		case Type::ZIP: r_compiler.zip(range, nameGet(), target_dir, rootGet()); break;
	}
}
...
TargetArchive* TargetArchive::create(const ResourceObject& obj,
                                     const TargetArchiveCompiler& intpret,
                                     const char* name_src,
                                     const char* in_dir,
                                     const char* root,
                                     size_t id,
