...
{
	if(options.objectExists("create")) { m_create = Command(options.objectGet("create")); }

	if(options.objectExists("rename")) { m_rename = Command(options.objectGet("rename")); }
...
{
	m_create.nameSet("zip").argumentAppend("-@").argumentAppend("{target}");

	m_rename.nameSet("zipnote").argumentAppend("-w").argumentAppend("{target}");
