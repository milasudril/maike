...
{
	return Handle<TargetArchive>(
	   TargetArchive::create(obj, r_compiler, name_src, in_dir, root, id, line_count));
}
...
void TargetArchiveLoader::dependenciesGet(const char* name_src,
                                          const char* in_dir,
                                          const char* root,
                                          ...
                                          DependencyBuffer& deps) const
{
	...
	if(!rc.objectExists("contents")) { return; }

	auto contents = rc.objectGet("contents");
	if(contents.typeGet() != ResourceObject::Type::ARRAY) { return; }
...
void TargetArchiveLoader::targetsLoad(const char* name_src,
                                      const char* in_dir,
                                      Spider& spider,
