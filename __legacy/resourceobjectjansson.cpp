...
{
	if(reinterpret_cast<DataSink*>(eventhandler)->write(buffer, size) == size) { return 0; }
	return -1;
}


ResourceObjectJansson::ResourceObjectJansson(DataSource& readhandler): ResourceObject(s_vtable)
{
	json_error_t status;
	...
	if(m_handle == nullptr)
	{
		...
		{
			exceptionRaise(
			   ErrorMessage("#0;:#1;: error: #2;.", {readhandler.nameGet(), status.line, status.text}));
		}
		m_handle = json_object();
	}
}

ResourceObjectJansson::ResourceObjectJansson(const char* str): ResourceObject(s_vtable)
{
	m_handle = json_string(str);
	if(m_handle == nullptr) { exceptionRaise(ErrorMessage("Failed to store #0;", {str})); }
}

ResourceObjectJansson::ResourceObjectJansson(long long int x): ResourceObject(s_vtable)
{
...
ResourceObjectJansson::ResourceObjectJansson(double x): ResourceObject(s_vtable)
{
	m_handle = json_real(x);
	if(m_handle == nullptr) { exceptionRaise(ErrorMessage("Failed to store #0;", {x})); }
}
...
ResourceObjectJansson::ResourceObjectJansson(Type type): ResourceObject(s_vtable)
{
	switch(type)
	{
		case Type::ARRAY: m_handle = json_array(); break;
		case Type::OBJECT: m_handle = json_object(); break;
		default: m_handle = nullptr;
	}
	if(m_handle == nullptr) { exceptionRaise(ErrorMessage("Failed to create a JSON object", {})); }
}
...
ResourceObjectJansson::ResourceObjectJansson(void* handle): ResourceObject(s_vtable, handle)
{
...
{
	if(handle != nullptr) { json_decref(static_cast<json_t*>(handle)); }
}
...
ResourceObject::Type ResourceObjectJansson::typeGetImpl(const void* handle)
{
	switch(json_typeof(static_cast<const json_t*>(handle)))
	{
		...
		case JSON_REAL: return Type::FLOAT;
...
ResourceObject ResourceObjectJansson::objectGetImpl(const void* handle, const char* name)
{
	...
	if(result == NULL) { exceptionRaise(ErrorMessage("Could not get child object \"#0;\".", {name})); }
...
const char* ResourceObjectJansson::stringGetImpl(const void* handle)
{
	...
	if(ret == NULL) { exceptionRaise(ErrorMessage("Current resource object is not a string.", {})); }
	return ret;
}
...
double ResourceObjectJansson::floatGetImpl(const void* handle)
{
	...
	{
		case Type::INTEGER: return static_cast<double>(integerGetImpl(handle));
		case Type::FLOAT: return json_real_value(static_cast<const json_t*>(handle));
		default: exceptionRaise(ErrorMessage("Current resource object is not an integer.", {}));
	}
	return 0;
}
...
void ResourceObjectJansson::objectAppendImpl(void* handle, ResourceObject&& object)
{
	...
	if(json_array_append_new(static_cast<json_t*>(handle), static_cast<json_t*>(obj->m_handle)) == -1)
	{
...
{
	auto obj = reinterpret_cast<ResourceObjectJansson*>(&object);
	if(json_object_set_new(static_cast<json_t*>(handle), key, static_cast<json_t*>(obj->m_handle))
	   == -1)
	{
...
{
	json_dump_callback(static_cast<const json_t*>(handle),
	                   storeCallback,
	                   &sink,
	                   JSON_COMPACT | JSON_INDENT(4) | JSON_SORT_KEYS);
}

const ResourceObjectJansson::Vtable ResourceObjectJansson::s_vtable{destroy,
                                                                    createImpl,
                                                                    createImpl,
                                                                    createImpl,
                                                                    createImpl,
...
const ResourceObjectJansson::Iterator::Vtable ResourceObjectJansson::Iterator::s_vtable{
   destroyImpl, nextImpl, getImpl, endAtImpl};
