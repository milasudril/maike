...
public:
	...
	Twins<const TreeHandle*> find(const Stringkey& name) const noexcept
	{
		...
		if(i == m_tree.end()) { return {nullptr, nullptr}; }
		auto& tree = i->second;
...
{
	if(dep_in.itemCountGet() > 0) { dep.objectSet("ref", ResourceObjectJansson(dep_in.itemGet(0))); }
	if(dep_in.itemCountGet() > 1) { dep.objectSet("rel", ResourceObjectJansson(dep_in.itemGet(1))); }
	else
	{
		...
		switch(state)
		{
			case State::INIT:
				switch(ch_in)
				{
					case '\r': break;
					case '\n':
						if(str == "#ifdef __WAND__") { state = State::SPELL; }
						else
						{
			...
			case State::SPELL:
				if(ch_in >= 0 && ch_in <= ' ') { break; }
				switch(ch_in)
				{
					case '#':
					{
						if(!tree_stack.empty()) { throw "Spell error: unterminated spell tree"; }
					...
					break;
					case '^': state = State::SPELL_ESC; break;
					case '[':
					{
					...
					case ';':
						...
						break;
					case ']':
						if(tree_stack.empty()) { throw "Spell error: too many ']'"; }
						if(str.size() != 0) { tree_current->itemAppend(str); }
...
int main(int argc, char** argv)
{
	try
	{
		...
		if(!FileUtils::exists(out_dir)) { FileUtils::mkdir(out_dir); }

		--argc;
		++argv;
		while(argc != 0)
		{
			if(FileInfo(*argv).typeGet() != FileInfo::Type::DIRECTORY) { process(*argv, out_dir); }
			--argc;
			++argv;
		}
	}
