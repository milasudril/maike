//@	{"targets":[{"name":"targetbashloader.o","type":"object"}]}

#include "targetbashloader.hpp"
#include "targetbash.hpp"
...
TargetBashLoader::TargetBashLoader(const TargetBashInterpreter& intpret): r_intpret(intpret)
{
}

Handle<Target> TargetBashLoader::targetCreate(const ResourceObject& obj,
                                              const char* name_src,
                                              const char* in_dir,
                                              const char* root,
                                              size_t id,
...
{
	return Handle<TargetBash>(
	   TargetBash::create(obj, r_intpret, name_src, in_dir, root, id, line_count));
}
...
void TargetBashLoader::dependenciesExtraGet(const char* name_src,
                                            const char* in_dir,
                                            const char* root,
...
void TargetBashLoader::targetsLoad(const char* name_src,
                                   const char* in_dir,
                                   Spider& spider,
