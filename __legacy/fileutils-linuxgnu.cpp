...
bool FileUtils::newer(const char* file_a, const char* file_b)
{
	...
	{
		exceptionRaise(
		   ErrorMessage("Error: None of the files #0;, and #1; are accessible.\n#0;: #2;.\n#1;: #3;.",
		                {file_a,
		                 file_b,
...
void FileUtils::copy(const char* source, const char* dest)
{
	...
	if(fstat(source_fd.get(), &source_stat) == -1)
	{
		unlink(dest);
		exceptionRaise(
		   ErrorMessage("stat error: #0;", {static_cast<const char*>(Maike::strerror(errno))}));
	}
...
void FileUtils::copyFilter(const char* source, const char* dest, const char* comment_line_regexp)
{
	{
		WriteBuffer wb(StdStream::output());
...
void FileUtils::removeTree(const char* name, const std::set<Stringkey>& keeplist)
{
	WriteBuffer wb(StdStream::output());
		...
		if(rmdir(node.first.c_str()) == -1)
		{
			...
			switch(res)
			{
				...
				case ENOTDIR:
					...
					{
						exceptionRaise(
						   ErrorMessage("It was not possible to remove #0: ",
				...
				case EEXIST:
				{
					...
					while((entry = dir.read()) != nullptr)
					{
						...
						if(keeplist.find(Stringkey(elem.c_str())) == keeplist.end()) { nodes.push({elem, 0}); }
					}
				}
				break;
				default:
...
static bool find_in_pathvar(const char* str, const char* path)
{
	...
	while(1)
	{
		...
		{
			case ':':
				if(FileUtils::exists(dircat(p, str).c_str())) { return 1; }
				p.clear();
				break;
			case '\0':
				if(FileUtils::exists(dircat(p, str).c_str())) { return 1; }
