...
{
	WriteBuffer wb(sink);
	wb.write(ProjectInfo::name())
	   .write(" version ")
	   .write(ProjectInfo::revision())
	   .write("\nThis Maike was compiled on ")
	   .write(ProjectInfo::compilationDate())
	   .write("\n");
}
...
void Maike::about(DataSink& sink)
{
	versionPrint(sink);
	WriteBuffer wb(sink);
	wb.write("Copyright (C) ")
	   .write(ProjectInfo::years())
	   .write(" ")
	   .write(ProjectInfo::author())
	   .write("\n\n")
...
void Maike::init(DataSink& standard_output, DataSink& standard_error, ExceptionHandler eh)
{
	init(eh);
...
static bool findString(const char* filename, const char* str)
{
	...
	while(!buffer.eof())
	{
		...
		if(v_str == '\0') { return 1; }
		if(v_str == ch_in) { ++pos_current; }
		else
		{
			pos_current = pos_init;
			if(*pos_current == ch_in) { ++pos_current; }
		}
	}
	return 0;
}
...
{
	if(!isMaike()) { about(StdStream::error()); }
	return new Session;
}
...
void Maike::hookRegister(Session& maike,
                         const char* name_plugin,
...
namespace
{
	...
	public:
		...
		int operator()(const DependencyGraph&, const Target& target)
		{
			...
			if(*description != '\0') { r_wb.write(" -- ").write(description); }
			r_wb.write("\n");
			return 0;
		}
                     ...
                     size_t id_min)
{
	...
	while(deps.first != deps.second)
	{
		...
		if(visited[id] == 0
		   && (full || deps.first->relationGet() != Dependency::Relation::IMPLEMENTATION))
...
static void buildBranch(Target& target, const char* target_dir, const Twins<size_t>& id_range)
{
	...
	while(deps.first != deps.second)
	{
		...
		if(target != nullptr)
		{
			...
			Twins<const Dependency*> deps_rel_full(
			   dependency_list_full.data(), dependency_list_full.data() + dependency_list_full.size());
...
namespace
{
	...
	public:
		...
		{
			auto x = target.dependenciesInverseGet();
			if(x.second - x.first == 0) { buildBranch(target, r_target_dir, id_range); }
			return 0;
		}
...
void Maike::targetsCompile(Session& maike)
{
	...
	{
		exceptionRaise(
		   ErrorMessage("I could not identify any targets "
...
void Maike::graphDump(const Session& maike,
                      GraphEdgeWriter& writer,
                      const char* target_start,
                      uint8_t* targets_visited,
                      size_t id_min)
{
...
void Maike::graphInvDump(const Session& maike,
                         GraphEdgeWriter& writer,
                         const char* target_start,
                         uint8_t* targets_visited,
                         size_t id_min)
...
void Maike::targetDump(const Session& maike, ResourceObject& db, const char* target_name)
{
...
static void targetDumpTSV(const Target& target, WriteBuffer& wb)
{
	...
	wb.write(target.idGet())
	   .write("\t")
	   .write(target.nameGet())
	   .write("\t")
	   .write(target.sourceNameGet())
	   .write("\t")
	   .write(static_cast<size_t>(deps_inv.second - deps_inv.first))
	   .write("\t")
	   .write(static_cast<size_t>(deps.second - deps.first))
	   .write("\t")
	   .write(target.compilationTimeGet())
	   .write("\t")
	   .write(target.lineCountGet())
	   .write("\n");
}
...
static void deleteBranch(Target& target, const char* target_dir, const Twins<size_t>& id_range)
{
	std::vector<Dependency> dependency_list_full;
	toposort(target, dependency_list_full, id_range, 1);
	...
	while(deps.first != deps.second)
	{
		...
		if(target != nullptr) { FileUtils::remove(dircat(target_dir, target->nameGet()).c_str()); }
	}
}
...
namespace
{
	...
	public:
		...
		{
			auto x = target.dependenciesInverseGet();
			if(x.second - x.first == 0) { deleteBranch(target, r_target_dir, id_range); }
			return 0;
		}
