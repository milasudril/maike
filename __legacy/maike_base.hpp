...
namespace Maike
{
	 ...
	 *
	 */
	PRIVATE inline std::unique_ptr<Session, decltype(&sessionDestroy)> sessionCreate()
	{
	 ...
	 *
	 */
	EXPORT void init(DataSink& standard_output, DataSink& standard_error, ExceptionHandler eh);
	...
	EXPORT void
	hookRegister(Session& maike, const char* name_plugin, Twins<const char* const*> filename_exts);
	...
	EXPORT void targetDumpTSV(const Session& maike, DataSink& sink, const char* target_name);
	 ...
	 *
	 */
	EXPORT void graphDump(const Session& maike,
	 ...
	 *
	 */
	EXPORT void graphInvDump(const Session& maike,
	                         GraphEdgeWriter& writer,
	                         const char* target_start,
	                         uint8_t* targets_visited,
	                         size_t id_min);
