...
namespace
{
	...
	{
	public:
		TargetsExternalDump(ResourceObject& libs,
		...
		int operator()(const DependencyGraph&, const Target& target)
		{
			...
			if(ptr != nullptr)
			{
				switch(ptr->relation())
				{
					case Dependency::Relation::EXTERNAL: r_libs.objectAppend(r_libs.create(ptr->nameGet())); break;
					case Dependency::Relation::TOOL: r_tools.objectAppend(r_tools.create(ptr->nameGet())); break;
					case Dependency::Relation::RUNTIME: r_rt.objectAppend(r_rt.create(ptr->nameGet())); break;
