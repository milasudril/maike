...
Handle<Target> TargetTrivialLoader::targetCreate(
   const ResourceObject&, const char*, const char*, const char*, size_t, size_t) const
{
	return Handle<TargetTrivial>(nullptr);
}

void TargetTrivialLoader::targetsLoad(const char* name_src,
                                      const char* in_dir,
                                      Spider&,
                                      ...
                                      Target_FactoryDelegator& factory) const
{
	...
	Handle<TargetTrivial> target{TargetTrivial::create(
	   r_opts.commandGet(), targetname.c_str(), name_src, in_dir, factory.rootGet(), factory.idGet())};
