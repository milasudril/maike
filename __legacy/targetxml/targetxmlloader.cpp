...
{
	configClear();
	m_filter.nameSet("xsltproc").argumentAppend("{depfile}").argumentAppend("{source}");
	return *this;
}

TargetXMLLoader& TargetXMLLoader::configAppend(const ResourceObject& config)
{
	if(config.objectExists("filter")) { m_filter = Command(config.objectGet("filter")); }
	return *this;
}
...
{
	return Handle<TargetXML>(TargetXML::create(obj, name_src, in_dir, root, id, line_count));
}
...
void TargetXMLLoader::targetsLoad(const char* name_src,
                                  const char* in_dir,
                                  Spider& spider,
                                  ...
                                  Target_FactoryDelegator& factory) const
{
	...
	depfile += ".deps";
	if(!FileUtils::exists(depfile.c_str())) { return; }
