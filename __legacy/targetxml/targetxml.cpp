                            ...
                            const char* target_dir)
{
	...
	m_cmd.argumentsProcess([&args](const char* str) { args.push_back(str); });
	...
	dependenciesProcess(
	   target_dir, dependencies(), USE_INCLUDE_EXTRA, [&deps](const char* name, Dependency::Relation) {
		   deps.push_back(std::string(name));
		   return true;
	   });

	std::for_each(
	   deps.begin(), deps.end(), [&args](const std::string& str) { args.push_back(str.c_str()); });

	Pipe pipe(m_cmd.nameGet(), {args.data(), args.data() + args.size()}, Pipe::REDIRECT_STDERR);
	...
	if(processFailed(ret))
	{
		if(ret.killedWas())
		{
			exceptionRaise(ErrorMessage("#0;: The compiler was killed with signal #1;.",
			                            {sourceNameGet(), ret.errorCodeGet()}));
		}
		else
		{
			exceptionRaise(ErrorMessage("#0;: The compiler returned status code #1;.",
			                            {sourceNameGet(), ret.errorCodeGet()}));
		}
	}
}
...
TargetXML* TargetXML::create(const ResourceObject& obj,
                             const char* name_src,
                             const char* in_dir,
                             const char* root,
                             size_t id,
