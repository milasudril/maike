...
static void
stringArrayBuild(const ResourceObject& array, std::vector<std::string>& ret, Validator&& valid)
{
	auto N = array.objectCountGet();
	for(decltype(N) k = 0; k < N; ++k)
	{
		auto str = static_cast<const char*>(array.objectGet(k));
		if(valid(str)) { ret.push_back(std::string(str)); }
	}
}
...
TargetCxxOptions& TargetCxxOptions::configAppend(const ResourceObject& cxxoptions)
{
	...
	{
		stringArrayBuild(cxxoptions.objectGet("includedir"), m_includedir, [this](const char* testval) {
	...
	{
		stringArrayBuild(
		   cxxoptions.objectGet("includedir_noscan"), m_includedir_noscan, [this](const char* testval) {
	...
	{
		stringArrayBuild(cxxoptions.objectGet("libdir"), m_libdir, [this](const char* testval) {
	...
	{
		stringArrayBuild(cxxoptions.objectGet("iquote"), m_iquote, [this](const char* testval) {
	...
	if(cxxoptions.objectExists("cxxversion_min"))
	{
		m_cxxversion_min = std::max(m_cxxversion_min,
		                            static_cast<unsigned long long int>(static_cast<long long int>(
		                               cxxoptions.objectGet("cxxversion_min"))));
	}
	...
	if(cxxoptions.objectExists("cxxversion_max"))
	{
		m_cxxversion_max = std::min(m_cxxversion_max,
		                            static_cast<unsigned long long int>(static_cast<long long int>(
		                               cxxoptions.objectGet("cxxversion_max"))));
	}
	...
	{
		stringArrayBuild(
		   cxxoptions.objectGet("cflags_extra"), m_cflags_extra, [this](const char* testval) {
	...
	{
		m_includedir_format =
		   std::string(static_cast<const char*>(cxxoptions.objectGet("includedir_format")));
	}
...
TargetCxxOptions& TargetCxxOptions::includedirNoscanAppend(const char* dir)
{
	...
	if(m_includedir_noscan_dup.insert(key).second) { m_includedir_noscan.push_back(std::string(dir)); }
	return *this;
}

TargetCxxOptions& TargetCxxOptions::configAppend(const TargetCxxOptions& cxxoptions)
{
	append(m_includedir, cxxoptions.m_includedir, [this](const std::string& testval) {
		return m_includedir_dup.insert(Stringkey(testval.c_str())).second;
	});
	append(m_includedir_noscan, cxxoptions.m_includedir_noscan, [this](const std::string& testval) {
		return m_includedir_noscan_dup.insert(Stringkey(testval.c_str())).second;
	});
	append(m_libdir, cxxoptions.m_libdir, [this](const std::string& testval) {
		return m_libdir_dup.insert(Stringkey(testval.c_str())).second;
	});
	append(m_cflags_extra, cxxoptions.m_cflags_extra, [this](const std::string& testval) {
		return m_cflags_extra_dup.insert(Stringkey(testval.c_str())).second;
	});
	append(m_iquote, cxxoptions.m_iquote, [this](const std::string& testval) {
	...
	if(m_cxxversion_max < m_cxxversion_min)
	{ exceptionRaise(ErrorMessage("cxxoptions: Inconsistent C++ version requirements", {})); }
	if(cxxoptions.m_stdprefix.size() != 0) { m_stdprefix = cxxoptions.m_stdprefix; }

	if(cxxoptions.m_cflags_format.size() != 0) { m_cflags_format = cxxoptions.m_cflags_format; }


	if(cxxoptions.m_includedir_format.size() != 0)
	{ m_includedir_format = cxxoptions.m_includedir_format; }

	if(cxxoptions.m_libdir_format.size() != 0) { m_libdir_format = cxxoptions.m_libdir_format; }

	if(cxxoptions.m_libext_format.size() != 0) { m_libext_format = cxxoptions.m_libext_format; }

	if(cxxoptions.m_libint_format.size() != 0) { m_libint_format = cxxoptions.m_libint_format; }

	if(cxxoptions.m_iquote_format.size() != 0) { m_iquote_format = cxxoptions.m_iquote_format; }

	if(cxxoptions.m_include_format.size() != 0) { m_include_format = cxxoptions.m_include_format; }

	if(cxxoptions.m_mode.size() != 0) { m_mode = cxxoptions.m_mode; }
	if(m_mode.size() == 0) { m_mode = std::string("c++"); } //	make sure there is a valid mode set. #63


	if(cxxoptions.m_objcompile) { m_objcompile = cxxoptions.m_objcompile; }

	if(cxxoptions.m_appcompile) { m_appcompile = cxxoptions.m_appcompile; }

	if(cxxoptions.m_dllcompile) { m_dllcompile = cxxoptions.m_dllcompile; }

	if(cxxoptions.m_libcompile) { m_libcompile = cxxoptions.m_libcompile; }

	if(cxxoptions.m_versionquery) { m_versionquery = cxxoptions.m_versionquery; }

	if(cxxoptions.m_pkgconfig) { m_pkgconfig = cxxoptions.m_pkgconfig; }

	if(cxxoptions.m_autorun_launcher) { m_autorun_launcher = cxxoptions.m_autorun_launcher; }
...
TargetCxxOptions& TargetCxxOptions::configAppendDefault()
{
	...
	m_objcompile.nameSet("g++")
	   .argumentAppend("-c")
	   .argumentAppend("-g")
	   .argumentAppend("-fpic")
	   .argumentAppend("{cxxversion}")
	   .argumentAppend("-Wall")
	...
	m_libcompile.nameSet("ar")
	   .argumentAppend("rcs")
	   .argumentAppend("{target}")
	   .argumentAppend("{dependencies}");

	m_versionquery.argumentsClear();
	m_versionquery.nameSet("g++")
	   .argumentAppend("-E")
	   .argumentAppend("-dM")
	   .argumentAppend("-x")
	   .argumentAppend("c++")
...
{
	if(m_libdir_dup.insert(Stringkey(dir)).second) { m_libdir.push_back(std::string(dir)); }
	return *this;
}

TargetCxxOptions& TargetCxxOptions::iquoteAppend(const char* dir)
{
	if(m_iquote_dup.insert(Stringkey(dir)).second) { m_iquote.push_back(std::string(dir)); }
	return *this;
}
