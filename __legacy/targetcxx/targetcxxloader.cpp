...
namespace
{
	...
	{
	public:
		TagExtractor(DataSource& source): m_reader(source), m_state(State::NEWLINE), m_lines(0)
		{
		}
...
{
	return Handle<TargetCxx>(
	   TargetCxx::create(obj, r_compiler, name_src, in_dir, root, id, line_count));
}


void TargetCxxLoader::dependenciesExtraGet(const char* name_src,
                                           const char* in_dir,
                                           const char* root,
...
{
	std::string
	resolveInclude(const std::string& filename, Twins<const std::string*> paths, const char** in_dir)
	{
		if(in_dir != nullptr)
		{
			auto name_dep_full = dircat(*in_dir, filename);
			if(FileUtils::exists(name_dep_full.c_str())) { return std::move(name_dep_full); }
		}
...
void TargetCxxLoader::dependenciesGet(const char* name_src,
                                      const char* in_dir,
                                      const char* root,
                                      ResourceObject::Reader,
			...
			case Mode::NORMAL:
				switch(tok_in.type)
				{
					case TargetCxxPPTokenizer::Token::Type::DIRECTIVE:
						if(tok_in.value == "include") { mode = Mode::INCLUDE; }
						break;
					default: break;
				}
				break;
...
void TargetCxxLoader::targetsLoad(const char* name_src,
                                  const char* in_dir,
                                  Spider& spider,
