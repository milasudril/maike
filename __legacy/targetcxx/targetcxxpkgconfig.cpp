...
PRIVATE void outputRead(Pipe& pipe, const char* delim_seq, const ItemSink& sink)
{
	...
	{
		if(str.size() != 0) { sink(str.c_str()); }
	}
}

typedef ParameterSetMapFixed<Stringkey("action"), Stringkey("libname")> PkgConfigParams;

template<class ItemSink>
PRIVATE ProcessExitStatus pipeExecute(const Command& cmd,
                                      const PkgConfigParams& params,
                                      const char* delim_seq,
                                      const ItemSink& sink)
{
...
PkgConfigRequest::PkgConfigRequest(const Command& cmd, const char* libname, const char* context)
{
	...
	if(processFailed(pipeExecute(
	      cmd, params, " -I", [this](const char* str) { m_incdir.push_back(std::string(str)); })))
	{
		exceptionRaise(
		   ErrorMessage("#0;: It was not possible to find information about "
		                "the library #1;.",
		                {context, libname}));
	}
	...
	if(processFailed(pipeExecute(
	      cmd, params, " -", [this](const char* str) { m_cflags.push_back(std::string(str)); })))
	{
		exceptionRaise(
		   ErrorMessage("#0;: It was not possible to find information about "
		                "the library #1;.",
	...
	if(processFailed(pipeExecute(
	      cmd, params, " -L", [this](const char* str) { m_libdir.push_back(std::string(str)); })))
	{
		exceptionRaise(
		   ErrorMessage("#0;: It was not possible to find information about "
		                "the library #1;.",
