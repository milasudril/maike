...
{
	if(strcmp(mode, "c++") == 0 || strcmp(mode, "C++") == 0) { return "__cplusplus"; }
...
static unsigned long long int cxxversionDefaultGet(Pipe& versionget, const char* mode)
{
	...
	while(cxxtok.read(tok))
	{
		switch(state)
		{
			case Mode::NORMAL:
				switch(tok.type)
				{
					case TargetCxxPPTokenizer::Token::Type::DIRECTIVE:
						if(tok.value == "define") { state = Mode::DEFINE; }
						break;
					default: break;
				}
				break;
			case Mode::DEFINE:
				if(tok.value == look_for) { state = Mode::DONE; }
				else
				{
					state = Mode::NORMAL;
				}
			...
			case Mode::DONE: return atoll(tok.value.c_str());
		}
	}
	return 0;
}
...
static unsigned long long int cxxversionDefaultGet(const Command& versionquery,
                                                   ...
                                                   const char* mode)
{
	auto versionget = versionquery.execute(Pipe::REDIRECT_STDOUT | Pipe::REDIRECT_STDERR, paramset);
	...
	if(processFailed(status))
	{
		...
		{
			exceptionRaise(
			   ErrorMessage("It was not possible to determine the "
			                "default C++ version. The compiler was killed with signal #0;",
			                {status.errorCodeGet()}));
		}
		else
		{
			exceptionRaise(
			   ErrorMessage("It was not possible to determine the "
			                "default C++ version. The compiler returned status code #0;",
			                {status.errorCodeGet()}));
		}
	}
	...
	{
		exceptionRaise(
		   ErrorMessage("The current language (#0;) appears to be "
		                "unsupported by the chosen compiler.",
		                {mode}));
	}
...
typedef ParameterSetMapFixed<Stringkey("cxxversion"),
...
static const char* cxxNameGet(unsigned long long int cxxversion, const char* mode)
{
	...
	{
		if(cxxversion < 201103L) { return "c++03"; }
		if(cxxversion < 201402L) { return "c++11"; }
		if(cxxversion > 201402L) { return "c++17"; }
		return "c++14";
	}
	else if(strcmp(mode, "C") == 0 || strcmp(mode, "c") == 0)
	{
		if(cxxversion < 199901) { return "c89"; }
		if(cxxversion < 201102) { return "c99"; }
		return "c11";
	}
	exceptionRaise(ErrorMessage("The current language (#0;) is not supported.", {mode}));
}
...
unsigned long long int TargetCxxCompiler::cxxversionDefaultGet(const char* mode) const
{
	...
	{
		auto p = &r_paramset;
		m_cxxversion_default = ::cxxversionDefaultGet(r_options.versionqueryGet(), {p, p + 1}, mode);
	}
                                     ...
                                     const TargetCxxOptions& options)
{
	...
	{
		case TargetCxxCompiler::FileUsage::NORMAL: return std::string(fileinfo.filename);
...
static void argvBuild(std::vector<std::string>& argv,
                      Twins<const std::string*> strings_raw,
                                ...
                                const TargetCxxOptions& options_extra) const
{
	...
	{
		cxxparams.get<Stringkey("cxxversion")>().push_back(
		   cxxVersionString(options_result.stdprefixGet(), cxxversion_min, options_result.modeGet()));
	}
	if(cxxversion_max < cxxversionDefaultGet(options_result.modeGet()))
	{
		cxxparams.get<Stringkey("cxxversion")>().push_back(
		   cxxVersionString(options_result.stdprefixGet(), cxxversion_max, options_result.modeGet()));
	}


	argvBuild(cxxparams.get<Stringkey("includedir")>(),
	          options_result.includedirGet(),
	          options_result.includedirFormatGet());
	argvBuild(cxxparams.get<Stringkey("includedir")>(),
	          options_result.includedirNoscanGet(),
	          options_result.includedirFormatGet());
	argvBuild(cxxparams.get<Stringkey("cflags_extra")>(),
	          options_result.cflagsExtraGet(),
	          options_result.cflagsFormatGet());
	argvBuild(cxxparams.get<Stringkey("iquote")>(),
	          options_result.iquoteGet(),
	          options_result.iquoteFormatGet());
	...
	if(dependencies.first != dependencies.second)
	{
		...
		do
		{
			...
			if(str.size() != 0) { deps.push_back(std::move(str)); }
			++ptr;
		} while(ptr != dependencies.second);
		if(deps.size() != 0)
		{
			argvBuild(cxxparams.get<Stringkey("libdir")>(),
			          options_result.libdirGet(),
			          options_result.libdirFormatGet());
		}
		...
		do
		{
			...
			if(str.size() != 0) { includes.push_back(std::move(str)); }
			++ptr;
		} while(ptr != dependencies.second);
	}

	const ParameterSet* paramset[] = {r_paramset, &cxxparams};
	auto compiler = cmd.execute(Pipe::REDIRECT_STDERR, {paramset, paramset + 2});
	...
	if(processFailed(res))
	{
		...
		{
			exceptionRaise(
			   ErrorMessage("#0;: It was not possible to generate #1;. "
			                "The compiler was killed with signal #2;",
			                {source, dest, res.errorCodeGet()}));
		}
		else
		{
			exceptionRaise(
			   ErrorMessage("#0;: It was not possible to generate #1;. "
			                "The compiler returned status code #2;",
			                {source, dest, res.errorCodeGet()}));
		}
	}
}

void TargetCxxCompiler::compileObject(const char* source,
                                      const char* in_dir,
                                      Twins<const FileInfo*> dependencies,
                                      const char* dest,
                                      const TargetCxxOptions& options_extra) const
...
const PkgConfigRequest& TargetCxxCompiler::pkgconfigAsk(const char* library,
                                                        const char* context) const
{
	auto key = Stringkey(library);
	auto i = m_pkgconfig_cache.find(key);
	if(i != m_pkgconfig_cache.end()) { return i->second; }

	auto ip =
	   m_pkgconfig_cache.insert({key, PkgConfigRequest(r_options.pkgconfigGet(), library, context)});
	return ip.first->second;
}


static void runTarget(const Command& cmd,
...
void TargetCxxCompiler::runTargetApp(const char* source_name,
                                     const char* in_dir,
                                     const char* appname,
                                     const TargetCxxOptions& options_extra) const
{
	...
	if(!cmd) { cmd.nameSet(appname); }

	runTarget(cmd, source_name, in_dir, appname, *r_paramset);
}

void TargetCxxCompiler::runTargetDll(const char* source_name,
                                     const char* in_dir,
                                     const char* appname,
                                     const TargetCxxOptions& options_extra) const
{
