...
static TargetCxx::Type type(const char* name_src, const char* type)
{
	...
	if(key == Stringkey("object")) { return TargetCxx::Type::OBJECT; }

	if(key == Stringkey("include")) { return TargetCxx::Type::INCLUDE; }

	if(key == Stringkey("application")) { return TargetCxx::Type::APPLICATION; }

	if(key == Stringkey("lib_dynamic")) { return TargetCxx::Type::LIB_DYNAMIC; }

	if(key == Stringkey("lib_static")) { return TargetCxx::Type::LIB_STATIC; }

	if(key == Stringkey("include_lib")) { return TargetCxx::Type::INCLUDE_LIB; }
...
static const char* type(TargetCxx::Type type)
{
	switch(type)
	{
		case TargetCxx::Type::OBJECT: return "object";
		case TargetCxx::Type::INCLUDE: return "include";
		case TargetCxx::Type::APPLICATION: return "application";
		case TargetCxx::Type::LIB_DYNAMIC: return "lib_dynamic";
		case TargetCxx::Type::LIB_STATIC: return "lib_static";
		case TargetCxx::Type::INCLUDE_LIB: return "include_lib";
	}
	return nullptr;
}
...
void TargetCxx::pkgconfig(const ResourceObject& pkgconfig_libs)
{
	...
	for(decltype(N) k = 0; k < N; ++k)
	{
		r_compiler.pkgconfigAsk(static_cast<const char*>(pkgconfig_libs.objectGet(k)), sourceNameGet())
		   .optionsPush(m_options_extra, m_options_extra_local)
		   .dependenciesPush(*this);
	}
}
...
{
	if(*a == '\0') { return chooseFirstNonEmpty(vals...); }
	return a;
}
                                           ...
                                           const char* root)
{
	...
	switch(targetType)
	{
		...
		case TargetCxx::Type::LIB_STATIC:
			...
			break;
		case TargetCxx::Type::INCLUDE_LIB: break;
		case TargetCxx::Type::INCLUDE: break;
	}
	return Dependency(depname, root, Dependency::Relation::TOOL);
}

TargetCxx::TargetCxx(const ResourceObject& obj,
   ...
   TargetBase(obj, name_src, in_dir, root, id, line_count), m_autorun(0), r_compiler(compiler)
{
	...
	if(obj.objectExists("pkgconfig_libs")) { pkgconfig(obj.objectGet("pkgconfig_libs")); }
	...
	if(obj.objectExists("autorun"))
	{
		...
		{
			exceptionRaise(ErrorMessage{
			   "#0;: Only applications and dynamic libraries can have the autorun flag", {name_src}});
		}
		...
		if(m_autorun)
		{
			...
			if(*depname != '\0') { dependencyAdd(Dependency(depname, root, Dependency::Relation::TOOL)); }
		}
	}
	...
	{
		dependencyAdd(makeDependencyOnCompiler(
		   m_options_extra_local, m_options_extra, optionsCompiler, m_type, root));
	}
}
...
bool TargetCxx::generated() const noexcept
{
	switch(m_type)
	{
		...
		case Type::INCLUDE_LIB: return 1;
		case Type::INCLUDE: return 0;
	}
	return 0;
}
...
static void optionsCollect(Twins<const Dependency*> deps, TargetCxxOptions& options_out)
{
	while(deps.first != deps.second)
	{
		switch(deps.first->relationGet())
		{
			...
			case Dependency::Relation::IMPLEMENTATION:
			{
				...
				if(target_rel) { options_out.configAppend(target_rel->optionsExtraGet()); }
			}
			break;
...
template<>
TargetCxxCompiler::FileUsage
map<TargetCxxCompiler::FileUsage, Dependency::Relation>(Dependency::Relation rel)
{
	switch(rel)
	{
			//	NORMAL,LIB_INTERNAL,LIB_EXTERNAL,INCLUDE_EXTRA
		case Dependency::Relation::IMPLEMENTATION: return TargetCxxCompiler::FileUsage::NORMAL;
		case Dependency::Relation::EXTERNAL: return TargetCxxCompiler::FileUsage::LIB_EXTERNAL;
		case Dependency::Relation::INCLUDE_EXTRA: return TargetCxxCompiler::FileUsage::INCLUDE_EXTRA;
		default: assert(!("Intenal error"));
	}
}
                            ...
                            const char* target_dir)
{
	...
	auto depstring_create = [&strings_temp, &depfiles](const char* depname, Dependency::Relation rel) {
		depfiles.push_back({push_back(std::string(depname), strings_temp).c_str(),
	...
	switch(m_type)
	{
		case Type::OBJECT:
		{
			...
			r_compiler.compileObject(
			   sourceNameGet(), inDirGet(), {deps_begin, deps_end}, name_full.c_str(), options_extra);
		}
		break;
		...
		case Type::APPLICATION:
		{
			...
			dependenciesProcess(target_dir, dependency_list, USE_INCLUDE_EXTRA, depstring_create);
			dependenciesProcess(
			   target_dir, dependency_list_full, USE_EXTERNAL | USE_IMPLEMENTATION, depstring_create);
			auto deps_begin = depfiles.data();
			auto deps_end = deps_begin + depfiles.size();
		...
		case Type::LIB_STATIC:
		{
			...
			auto deps_end = deps_begin + depfiles.size();
			std::reverse(deps_begin, deps_end);
			r_compiler.compileLibrary(
			   sourceNameGet(), inDirGet(), {deps_begin, deps_end}, name_full.c_str(), m_options_extra);
		}
		break;

		case Type::INCLUDE: break;
		default:
			WriteBuffer(StdStream::error())
			   .write(name_full.c_str())
			   .write(": Target type ")
			   .write(static_cast<size_t>(m_type))
			   .write(" not handled\n");
	}
}
                         ...
                         const char* target_dir) const
{
	...
	if(FileUtils::newer(sourceNameGet(), name_full.c_str())) { return 0; }
	...
	{
		case Type::OBJECT:
			return dependenciesProcess(target_dir,
			                           dependency_list,
			                           USE_INCLUDE_EXTRA | USE_INCLUDE | USE_MISC | USE_TOOL,
			                           up_to_date);

		case Type::INCLUDE_LIB:
			return dependenciesProcess(
			   target_dir, dependency_list, USE_INCLUDE | USE_INCLUDE_EXTRA, up_to_date);

		case Type::INCLUDE: return 1;
